...
//!
//! Gated on `ONESHOT_E2E=1` so a plain `cargo test` stays hermetic. Point at
//! a specific binary with `ONESHOT_CLAUDE_BIN=/path/to/claude` (required on
...
const BIN: &str = env!("CARGO_BIN_EXE_oneshot");

fn e2e_enabled() -> bool {
    std::env::var("ONESHOT_E2E").as_deref() == Ok("1")
}
...
fn text_mode_returns_answer() {
    if !e2e_enabled() {
        eprintln!("skipping (set ONESHOT_E2E=1)");
        return;
    }
    let out = run(&[
